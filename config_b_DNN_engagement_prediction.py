...
X1, X2 = np.meshgrid(
    np.arange(
        start=X_set[:, 0].min() - 45.0, stop=X_set[:, 0].max() + 45.0, step=0.0450
    ),
    np.arange(
        start=X_set[:, 1].min() - 0.01, stop=X_set[:, 1].max() + 0.01, step=0.0005
    ),
)
...
plt.contourf(
    ...
    loaded_model.predict(
        scx.transform(np.array([X1.ravel(), X2.ravel(), np.ones_like(X1.ravel())]).T)
    ).reshape(X1.shape),
    alpha=0.75,
...
plt.title(
    r"Deep Learning Based Centrifugal Clutch Engagement Prediction",
